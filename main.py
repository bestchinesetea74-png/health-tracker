...
class LightButton(MDBoxLayout):
    ...
    def __init__(self, text="", filled=False, color=None, on_release=None,
        ...
        base = color or ((0.20, 0.45, 0.55, 1) if filled else (0.22, 0.22, 0.24, 1))
        with self.canvas.before:
...
class LightIconButton(MDBoxLayout):
    ...
    def __init__(self, icon="", tonal=False, on_release=None, **kwargs):
        ...
        super().__init__(**kwargs)
        self._on_release = on_release
        base = (0.20, 0.45, 0.55, 0.35) if tonal else (0, 0, 0, 0)
        with self.canvas.before:
            self._col = Color(*base)
...
class LightTabItem(MDBoxLayout):
    ...
    def _apply_selected(self):
        self._col.rgba = (0.20, 0.45, 0.55, 0.35) if self.selected else (0, 0, 0, 0)
