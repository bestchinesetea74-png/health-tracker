...
class HealthTrackerData:
    ...
    ВАЖНО: этот класс НЕ наследуется от MDApp — это чисто логика,
    отделённая от интерфейса, чтобы больше не было конфликта имён
    с классом приложения HealthTrackerApp(MDApp) ниже.
    """
    ...
    def _perform_drive_upload(self):
        ...
        (потребует OAuth-авторизацию, отдельная задача под Android)."""
        if not os.path.exists("tracker_history.json"):
            return
        if MediaFileUpload is None:
            print("[Sync] Google API недоступен, аплоад пропущен.")
            return
...
class HealthTrackerApp(MDApp):
    """Твой рабочий UI-скелет (тот, что был на Windows), теперь подключённый
    к слою данных HealthTrackerData вместо просто демо-кнопки."""
