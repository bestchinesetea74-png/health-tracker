...
class GoogleDriveSync:
        ...
        try:
            ...
            if self.file_id:
                ...
                result = self.service.files().create(
                    body={'name': self.remote_filename}, media_body=media, fields='id'
                ).execute()
    ...
    def download_file_safe(self):
        ...
        сетевой, HTTP или JSON-парсинга. Вызывающая сторона (безопасная
        замена локального файла) — отдельным шагом."""
        if not self.service:
            return False, None
        try:
            if not self.file_id:
            ...
            raw = request.execute()  # bytes — целиком в память, локальный файл не тронут
