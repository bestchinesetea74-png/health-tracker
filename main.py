...
class HealthTrackerApp:
    ...
    def save_database(self):
        ...
        if os.path.exists("tracker_history.json"):
            try:
                with open("tracker_history.json", "r", encoding="utf-8") as f:
    ...
    def trigger_upload_with_debounce(self):
    ...
    def _perform_drive_upload(self):
        """Фактическая выгрузка файла на Google Диск через API."""
        if not os.path.exists("tracker_history.json"):
            return
        media = MediaFileUpload("tracker_history.json", mimetype='application/json')
        print("[Sync] Uploading tracker_history.json to Google Drive...")
