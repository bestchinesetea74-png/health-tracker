...
class TestSafeDownloadKeepsLocalFileIntact(unittest.TestCase):
    ...
    def test_network_exception_keeps_local_file_unchanged(self):
        ...
        sync.service.files().get_media().execute.side_effect = Exception("network timeout")
    ...
    def test_invalid_remote_json_keeps_local_file_unchanged(self):
        ...
        sync.service = MagicMock()
        sync.file_id = "fake-id"
        sync.service.files().get_media().execute.return_value = b"{not valid json"
    ...
    def test_valid_remote_json_can_replace_local_file(self):
        ...
        sync.service.files().get_media().execute.return_value = json.dumps(new_remote_data).encode("utf-8")
    ...
    def test_no_service_keeps_local_file_unchanged(self):
        # Например, credentials.json отсутствует — service не создан.
        sync = GoogleDriveSync.__new__(GoogleDriveSync)
        sync.service = None
        sync.file_id = None
